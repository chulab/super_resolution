...
def pad_last_to_size(tensor, shape):
  tensor_shape = tensor.shape.as_list()
  tensor_dims = len(tensor_shape)
  if len(shape) > tensor_dims:
    raise ValueError('len of `shape` must be <= dimensions of `tensor`')
  pads = [[0, 0]] * (tensor_dims - len(shape))
  pads += [[0, s - tensor_shape[tensor_dims-len(shape)+i]]
...
def fft_conv(tensor_a, tensor_b, mode):
  ...
  """
  tensor_size_a = tensor_a.shape.as_list()[-2:]
  tensor_size_b = tensor_b.shape.as_list()[-2:]
  ...
  tensor_a, tensor_b = pad_last_to_size(tensor_a, dft_size), pad_last_to_size(
    tensor_b, dft_size)
...
def _centered(tensor, newsize):
  ...
  tensor_shape = tensor.shape.as_list()
  currshape = tensor_shape[-len(newsize):]
  excess_shape = tensor_shape[:(len(tensor_shape)-len(newsize))]

  start_ind = [(cur - new) // 2 for cur, new in zip(currshape, newsize)]
  end_ind = [start + new for start, new in zip(start_ind, newsize)]
  myslice = [slice(end) for end in excess_shape] + \
    [slice(start, end) for start, end in zip(start_ind, end_ind)]
