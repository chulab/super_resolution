...
from tensor2tensor.models import image_transformer_2d, transformer
...
class SelectiveTransformer(t2t_model.T2TModel):
  ...
  def __init__(self,
    ...
    super(SelectiveTransformer, self).__init__(
      hparams,
      mode,
      problem_hparams,
      data_parallelism,
    ...
    # self.channel_emb = tf.get_variable(
    #   name="channel_emb",
    #   shape=[1, hparams.channel_size, image_pixels],
    ...
    self.conv_emb = []
    for i in range(hparams.channel_size):
      self.conv_emb.append(
  ...
  def bottom(self, features):
    ...
    if "targets" in features:
      transformed_features["targets"] = modalities.image_targets_bottom(
        features["targets"], self._hparams, self._problem_hparams.vocab_size["targets"]
      )
  ...
  def body(self, features):
    ...
    image_pixels = (hparams.example_shape
      // hparams.observation_pool_downsample) ** 2
    reshaped = tf.transpose(inputs, perm=[0, 3, 1, 2])
    reshaped = tf.reshape(reshaped, shape=[-1, reshaped.shape[1],
      reshaped.shape[2] * reshaped.shape[3]])
    batch_channel_emb = tf.ones([tf.shape(reshaped)[0], 1, 1]) * self.channel_emb
    attended_emb = reshaped
    for i in range(hparams.channel_layers):
      attended_emb = common_attention.multihead_attention(
        batch_channel_emb,
        attended_emb,
        None,
        image_pixels,
        image_pixels,
        image_pixels,
        hparams.example_shape // hparams.observation_pool_downsample,
        hparams.attention_dropout,
        name="channel_attention_{}".format(i)
      )
    attended_emb = tf.transpose(attended_emb, perm=[0, 2, 1])
    attended_emb = tf.reshape(attended_emb, shape=[-1, inputs.shape[1],
      inputs.shape[2], attended_emb.shape[2]])

    storage = []
    for image, conv in zip(tf.split(attended_emb, attended_emb.shape[-1], -1),
      self.conv_emb):
      storage.append(conv(image))
    # for image in tf.split(attended_emb, attended_emb.shape[-1], -1):
    #   storage.append(self.conv(image))
    processed_input = tf.concat(storage, axis=2)
    encoder_input = cia.prepare_encoder(processed_input, hparams, hparams.enc_attention_type)
