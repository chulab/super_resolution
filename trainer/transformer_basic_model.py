"""Defines transformer model used to learn reconstruction."""
...
from trainer import imgs_to_img_transformer as imgs2img

def make_hparams(vocab_size) -> tf.contrib.training.HParams:
  """Create a HParams object specifying model hyperparameters."""
  base = imgs2img.img2img_transformer2d_base()
  p_hparams = super_reso_problem_hparams(vocab_size, vocab_size, hparams)
  base.add_hparam("problem", p_hparams)
  return base


def network(input_layer, mode, params):
  """Defines network.
  ...
  with tf.variable_scope("Model"):
    network = image_transformer_2d.Img2imgTransformer(params, mode).apply(input_layer)
...
def model_fn(features, labels, mode, params):
  ...
  # observations = tf.reshape(observations, shape=[-1, shape[1], shape[2]])
  #
  # logging.info("observations after stacking {}".format(observations))

  # Run observations through Transformer.
  params.num_channels = feature.shape[-1]
  network = Imgs2imgTransformer(params, mode, params["problem"])
