...
from trainer import imgsel_transformer as imgs2img
...
def make_hparams() -> tf.contrib.training.HParams:
  ...
  hparams.add_hparam("channel_size", 8)
  hparams.add_hparam("channel_layers", 4)
...
def gpu_preprocess(observations, distributions, params):
  ...
  storage = []
  for freqs, ang_index in zip(tf.split(observations, observations.shape[1], 1),
    params.angle_indices):
    pooled = observation_pooling_layer.apply(tf.squeeze(freqs, 1))
    height = int(pooled.shape[1])
    width = int(pooled.shape[2])
    rotated = tf.contrib.image.rotate(pooled,
      -1 * params.observation_spec.angles[ang_index], interpolation='BILINEAR')
...
def model_fn(features, labels, mode, params):
  ...
  network = imgs2img.SelectiveTransformer(params, mode, params.problem)
  features_t = {
    "inputs": observations,
  ...
  predictions_quantized, _ = network.apply(features_t)
